...
//     char* m = HOTP_string;
    ...
    char m[8];
    m[7] = hotp == 1 ? 1 : ((t >> 56) & 0xff); // This is the hard-coded counter value.
    m[6] = hotp == 1 ? 0 : ((t >> 48) & 0xff); // This is the hard-coded counter value.
    m[5] = hotp == 1 ? 0 : ((t >> 40) & 0xff); // This is the hard-coded counter value.
    m[4] = hotp == 1 ? 0 : ((t >> 32) & 0xff); // This is the hard-coded counter value.
    m[3] = hotp == 1 ? 0 : ((t >> 24) & 0xff); // This is the hard-coded counter value.
    m[2] = hotp == 1 ? 0 : ((t >> 16) & 0xff); // This is the hard-coded counter value.
    m[1] = hotp == 1 ? 0 : ((t >> 8) & 0xff); // This is the hard-coded counter value.
    m[0] = hotp == 1 ? 0 : ((t >> 0) & 0xff); // This is the hard-coded counter value.
